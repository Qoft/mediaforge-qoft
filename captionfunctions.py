...
def speech(image, caption, tosavename=None):
    return htmlcap("captionhtml/speech.html", image, caption, tosavename)
...
def remove_white(file, widthd, heightd):
    width = file.size[0] 
    height = file.size[1] 
    for i in range(0,width):# process all pixels
        for j in range(0,height):
            if i > widthd or j > heightd:
                pass # do sex
            else:
                data = file.getpixel((i,j))
                if ((data[0]==255 and data[1]==255 and data[2]==255) or (data[0]==195 and data[1]==188 and data[2]==185)):
                    file.putpixel((i,j),(255, 255, 255, 0))
    return file

def speechbubble(file, _, tosavename=None):
    if tosavename is None:
        tosavename = temp_file("png")
    frontImage = Image.open("rendering/speechbubble.png")
    background = Image.open(file)
    final1 = Image.new("RGBA", background.size)
    frontImage = frontImage.resize((background.width, int(frontImage.height/3)))
    frontImage = frontImage.convert("RGBA")
    background = background.convert("RGBA")
    final1.paste(background, (0, 0))
    final1.paste(frontImage, (0, 0), frontImage)
    final1 = remove_white(final1, background.width, frontImage.height)
    final1.save(tosavename)
